...
serve(async (req) => {
  ...
  try {
    ...
    const journalText = (entries || []).map((e: any) => {
      const d = e.content
      if (!d) return ''
      if (e.day_number === 0) {
    ...
    const career = entries?.find((e: any) => e.day_number === 5)?.content?.career || '창업가'
...
응답 형식 (JSON만):
{
  ...
  "roadmap": [
    {"category": "도메인 학습", "phases": [
      {"period": "1개월", "goal": "구체적 목표", "milestone": "결과물"},
      {"period": "6개월", "goal": "구체적 목표", "milestone": "결과물"},
      {"period": "1년", "goal": "구체적 목표", "milestone": "결과물"},
      {"period": "5년", "goal": "구체적 목표", "milestone": "결과물"}
    ]},
...
roadmap의 각 goal과 milestone은 학생의 일지 내용을 반영해서 구체적으로 작성해주세요.`
